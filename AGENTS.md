...
- [analysis/](analysis/README.md) is the M3 Python processing package. It has a committed uv environment, DuckDB as the selected large-tabular engine, versioned spatial/source/lineage contracts, read-only input validators, CLI boundaries, and deterministic construction of the EPSG:3310 grid and per-cell water geometry from an explicit mask. M3 remains **In progress** because retrieval, cleaning, whale transfer, vessel aggregation, later derived outputs, and visual map inspection are not implemented.
