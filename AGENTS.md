...
- [analysis/](analysis/README.md) is the M3 Python processing package. It has a committed uv environment, DuckDB as the selected large-tabular engine, versioned spatial/source/lineage contracts, read-only input validators, a local one-artifact AIS retrieval-manifest boundary, a bounded local intake that partitions one author-supplied multi-date AccessAIS CSV or safe ZIP into deterministic daily cleaner inputs, resumable sequential cleaner-to-period-manifest orchestration, CLI boundaries, deterministic one-extract AIS cleaning, deterministic construction of the EPSG:3310 grid and per-cell water geometry from an explicit mask, and deterministic area-weighted whale-grid transfer. The one-day AccessAIS direct-CSV format and the new intake/orchestration path have been exercised successfully, while no real multi-date delivery has been exercised and independent transfer completeness, observational completeness, and safe monthly/full-period scaling remain unverified. The water grid and whale-grid transfer are tested and reproducible; two clean whale-transfer runs produced byte-identical output, and both exact derived artifacts were visually verified in QGIS 4.2.1 on 2026-08-27. M3 remains **In progress** because network and analytical-period AIS retrieval, vessel aggregation, exposure analysis, and later derived outputs are not implemented. Publication and deployment also remain unfinished.

The initial architecture ([ADR 0001](docs/decisions/0001-accept-initial-architecture.md)) is refined by the accepted hybrid GIS toolchain ([ADR 0015](docs/decisions/0015-adopt-a-hybrid-open-source-and-esri-gis-toolchain.md)): Python owns reproducible processing, QGIS owns local inspection and visual verification, and the public Next.js map retains direct Esri integration. ArcGIS Pro is optional and unnecessary for Version 1. The repository has a GitHub Actions CI workflow with stable `analysis` and `web` jobs for pull requests and pushes to `main`. The bootstrap workflow has not yet run on GitHub, and no main-branch ruleset or required status checks have been configured. CI does not deploy or publish anything. [tools/](tools/README.md) remains a data-discovery evidence utility, not the analysis package.
