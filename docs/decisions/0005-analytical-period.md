...
- **Volume — an order-of-magnitude planning estimate, not a measurement.** Scaling a 34-minute sample window to 24 hours across five sampled dates gives ≈0.4–0.6 million study-area records per day, so 153 days is **of order 10⁸ — roughly 60 to 90 million records**. Via the bulk route that means moving ≈56 GB of national daily files, based on the mean 369 MB compressed size across the five days actually sampled. **The retrieval route is an M3 decision** and is not settled by this record; the constraint that applies either way is in [../../data/README.md](../../data/README.md) — filter each day as it is retrieved, and never stage an entire national season locally.
