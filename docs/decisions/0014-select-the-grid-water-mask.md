...
- a separate **AIS observability or reporting-domain mask** describing where the
  land-receiver vessel input can support analysis.

They answer different questions. [ADR 0002](0002-southern-california-study-area-extent.md)
now accepts a receiver-station-based analytical domain. Using the observed edge
of the whale model for that purpose would still turn a modeling-product
coastline into a claim about radio coverage.
...
At the time of this decision, no separate authoritative shoreline artifact had
been registered or inspected for this project. Selecting one solely to construct this grid would add a
...
The grid water mask remains separate from the accepted AIS observability,
analytical-domain, and reporting-domain mask in ADR 0002. The grid is
...
- The accepted `receivers_50_nautical_miles` mask does not supersede this
  biological-support decision; it is separate geometry with separate
  provenance and meaning. Its distance is measured from the relevant NAIS
  reception stations, not from the coast.
- The accepted AIS mask must not erase or relabel grid water geometry. It is an
...
create areas with water geometry and no whale-model support. No candidate had
been inspected or registered for this slice, so choosing one would have silently added
a source and an untested rule. Later evidence registered CUSP for sensitivity
analysis, but accepted ADR 0002 uses receiver-station distance rather than a
coastline rule.
