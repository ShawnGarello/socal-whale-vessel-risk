...
**ADR 0018 is Accepted and M3 is Complete as of 2026-09-05**, on the criteria
evidence recorded in the [roadmap](roadmap.md). The 300-second/30-knot configuration is selected
...
scientifically validated, and publisher-transfer and observational completeness
remain `unverified`.
...
**Steps 1 to 3 are complete.** See "Production results" below for identities and
evidence. The remaining decision is the milestone flag itself.

1. Done. The production input was generated, after one retained resource-abort
   attempt, and repeated byte-identically in a separate location.
2. Done. Counts, identities, lineage, conservation, units and speed semantics
   were verified independently, and the exact activity and speed fields were
   inspected in QGIS with image checksums recorded.
3. Done. Owner documents are reconciled, the M3 criteria are assessed in the
   [roadmap](roadmap.md), and M3 is marked Complete. Two follow-ups did not gate
   the milestone and stay explicitly tracked under named owners: a reusable
   verification record with append-only/versioned lineage, carried to M8; and
   the unverified GDAL/Pyogrio read-back for publication compatibility with
   ArcGIS, carried to M5. Exposure calculation is M6 and out of scope.
...
in 93.79 seconds, and sdist/wheel build. They were re-run unchanged after the
production validation on 2026-09-05: lock check, formatting across 78 files,
lint, strict mypy over 37 source files, **417 tests** in 51.06 seconds, and the
sdist/wheel build. No web build was run; this session changed no web code.
...
reflect the completed production validation; historical sections below retain
the state at their recorded time.

Exposure calculation belongs to M6 and is out of scope. Nothing has been pushed
or merged.

### Production results, 2026-09-05

Generated by `whale_vessel_analysis.vessel_input_cli` through the resource
profiler with the recorded gates, from the ready period
`multiday-ais-17e982f999f7093945193378` on water grid `7229098c…`.
Input identity `vessel-input-5e590ff3d85ee7acb16e2fd1`.

| Bundle | Location | `vessel-grid.parquet` | `quality-report.json` | `run-metadata.json` |
|---|---|---|---|---|
| First | `data/derived/m3-production-vessel-first-attempt2/` | `5d3b1298…f688c0` | `4d0565af…bc9de7` | `799bc9c9…658fcb` |
| Repeat | `data/derived/m3-production-vessel-repeat-attempt2/` | `5d3b1298…f688c0` | `4d0565af…bc9de7` | `150dc573…807626` |

Full grid checksum
`5d3b12982f093e637ebda4a0fbd7ac4a1bb4756c6d1c1c2d3a696d2a0ef688c0`; quality
`4d0565af16c15fc9dc176db7b5b14cef99848e7bd48f1a3986dbaca1a5bc9de7`.
Parquet and quality bytes are identical across the two independent runs; only
lineage differs, which is the intended contract.

The first bundle ran 1,702.049 s with `target_completed`, preflight available
memory 3.85 GiB, minimum available 2.32 GiB, peak application RSS 1.331 GiB,
peak spill 1.075 GiB and zero stderr. The repeat was launched detached from the
agent harness after an initial attempt was killed by the harness at about 27
minutes for reported low memory while the profiler's own guard read
`within-thresholds` at 3.97 GiB available; that kill was not a project
resource-gate failure and its log is retained at
`data/interim/m3-production-vessel-repeat/run.log`.

Verification, `data/interim/m3-production-vessel-verification/report.json`:
`passed: true`. It independently reconstructed identity, physical units, speed
invariants, lineage and candidate parity for both bundles.

Independent checks performed against the output:

- 4,516 cells; 46 columns; observations 15,458,567 matching the period manifest.
- Retained 14,946,183 of 15,457,099 candidate segments; excluded 510,916, being
  461,769 maximum-gap and 49,147 implied-speed, which sum exactly.
- Distance conservation `passed`, per-group difference 0.0 m, maximum segment
  difference 1e-12 m; group allocations sum to the all-commercial total.
- All four vessel groups equal the retained 300/30 candidate
  `be3dc74d…5ec860` exactly, so the production path reuses the tested engine.
- Speed distance conserves: 2,031,132.165 + 52,948.904 + 421.427 =
  2,084,502.496 km. Segment classes 14,810,850 + 124,436 + 10,897 = 14,946,183.
- Water area from actual intersected geometry: 431 distinct partial areas from
  0.002163 to 25 km², 4,085 full-water cells, none above the nominal maximum;
  `water_area_m2`/`water_area_km2` and the density column agree to 0.000e+00.
- Speed means are null in exactly the 140 cells with `sog_available_km == 0`,
  and that equivalence holds for every row. One cell has zero vessel-km with a
  positive distinct-vessel count, the documented stationary-only case.

QGIS 4.2.1 inspection, offscreen, bound to grid checksum `5d3b1298…`:

| View | Field | Image SHA-256 |
|---|---|---|
| full-context | `vessel_km_all_commercial` | `f87d3129918fc3d7cc64f17f454390d2174ebc1b6bfb973c0f1bac8f5b8e330a` |
| shipping-corridors | `vessel_km_all_commercial` | `ae2c916609388963b410a599793b6ddc2c09577e2716c8b1aa14070b645f216f` |
| north-edge | `vessel_km_all_commercial` | `fbc3a74fed687625a2a94ab8f0bd4f5ca0c2cf408b2397b5dfc88f658d28a509` |
| south-edge | `vessel_km_all_commercial` | `b9677b9987e02aae91d09f97db609219c30a8255f213d53333a502fd408ad5c9` |
| shipping-corridors | `reported_sog_mean_knots_all_commercial` | `67ea88a4a095681009310eaf0e93d029f8c0ecff9f45dfba1f66b5990481e7cd` |

Images are under `data/interim/m3-production-qgis/`. The renderer reports
`visual_inspection_status: not_completed_by_rendering`; the readings below are
the separate human inspection of those exact images.

Read from the images: correct Southern California placement with islands as
excluded holes; highest values at the Los Angeles/Long Beach approach; coherent
Santa Barbara Channel and coastal corridors rather than noise; grid registered
to the coastline with no projection offset; boundary-adjacent cells visibly
lower, consistent with cleaned-extent censoring; and a speed field structurally
distinct from activity, lighter in port approaches and darker offshore, which is
what an intensive mean should look like. Offshore one-cell-tall streaks are
consistent with individual east-west transits, because at 300 s and 30 knots a
single segment spans at most about 4.6 km, less than one 5 km cell, so no single
inferred segment can cross several cells. These are interpretations of the
rendered evidence, not independent validation of the thresholds.
