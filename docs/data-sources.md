...
| **Custom extracts** | AccessAIS — <https://coast.noaa.gov/digitalcoast/tools/ais.html>. Read-only status, limit, and size-estimate endpoints were exercised on 2026-08-27. Author-controlled bounded 2024-07-15 and 2024-07-15 through 2024-07-16 deliveries were supplied as direct CSVs and exercised read-only on 2026-08-28 and 2026-09-01. See "Retrieval route" below |
...
**Accepted in [ADR 0017](decisions/0017-prefer-accessais-with-guarded-bulk-fallback.md).** The preferred route is five sequential author-submitted monthly AccessAIS extracts over the map/context bounds, with guarded one-day-at-a-time bulk retrieval as fallback only. The bounded one-day, overlapping two-day, seven-day, July monthly, August monthly, and September monthly AccessAIS direct-CSV deliveries passed their authorized local format, identity, header/date, cleaner-compatibility, and operational gates. The audited July gate authorizes sequential August--November calendar-month extracts under the same controls; August and September have now been processed under them. Independent transfer completeness, October--November/full-period safety, and complete bulk-day handling remain unresolved.
...
**Real overlapping two-day AccessAIS delivery inspected on 2026-09-01.** The
author reported retrieval on 2026-08-31, but the exact UTC retrieval timestamp
was not retained and is not inferred from filesystem modification time. The
immutable input was inspected in place and was not copied into this worktree.

| Delivery field | Observed value |
|---|---|
| Request | UTC dates 2024-07-15 through 2024-07-16; WGS 84 longitude −122 to −117 and latitude 32 to 35 |
| Route and container | Author-controlled AccessAIS; direct CSV |
| NOAA filename | `AIS_178822822548476721_896-1788228225861.csv` |
| Author-reported retrieval date | 2026-08-31; exact UTC retrieval timestamp not retained |
| Local byte identity | 115,791,285 bytes; SHA-256 `a6c673f37ccd01d30067c400452275b13f8c5299200777384a513bc46d6842a0` |
| Independent source metadata | No retained HTTP `Content-Length`, `ETag`, or ZIP CRC; independent transfer completeness remains `unverified` |
| CSV inspection | Exact NOAA 17-field rows; 1,135,408 rows; 582,419 on 15 July and 552,989 on 16 July; 0 malformed timestamps or coordinates; 0 rows outside the request or bounds |
| Timestamp bounds | `2024-07-15T00:00:00Z` through `2024-07-16T23:59:59Z`; not proof of transfer or observational completeness |
| Coordinate bounds | Longitude −121.99995 to −117.00026; latitude 32.00002 to 34.9999 |
...
period manifest. It reused 15 July, cleaned and recorded 16 July, and ended
...
**Real July monthly AccessAIS delivery exercised on 2026-09-03.** The immutable
author-supplied input was inspected and processed in place and was not copied
into this worktree.

| Delivery field | Observed value |
|---|---|
| Request | UTC dates 2024-07-01 through 2024-07-31; WGS 84 longitude -122 to -117 and latitude 32 to 35 |
| Route and container | Author-controlled AccessAIS; direct CSV |
| NOAA filename | `AIS_178840031620476771_233-1788400317272.csv` |
| Retrieval timestamp | Exact UTC retrieval timestamp not retained |
| Local byte identity | 1,827,867,349 bytes; SHA-256 `30b64b3733f391a614faab0311e419b8b5e7d2262d196d87606de57397c11169` |
| Independent source metadata | No retained HTTP `Content-Length`; publisher-side transfer completeness remains `unverified` |
| CSV inspection | Exact NOAA 17-field rows; 17,998,955 rows assigned across exactly all 31 requested dates; 0 malformed/unassignable timestamps; 0 valid rows outside the request |
| Processing result | 3,384,056 cleaned commercial observations; 31 compatible dates; 0 conflicts; 122 August--November dates missing |
| Stable identities | Delivery `accessais-period-c718fbfe6a3eb2d200ace41e`; period input `multiday-ais-d66b0637fb841469f4d585a5` |

Manifest-to-manifest comparison reproduced every canonical daily and cleaned
output identity for the overlapping 15--21 July seven-day evidence. The first
run and identical retry completed within the unchanged resource thresholds;
the retry skipped all 31 dates without regeneration. This verifies the local
identity, delivered-row/date reconciliation, cleaner compatibility, and bounded
execution of this exact July artifact. It does not establish that the publisher
...
**Still unverified.** Independent AccessAIS transfer completeness because no HTTP length or object validator was retained; download/range-resume behavior; October--November and full-period processing safety; complete bulk-file integrity; and any authoritative expected per-date record count. The seven-day, exact July monthly, exact August monthly, and exact September monthly operational gates passed, but three processed months cannot establish the remaining two months or the 153-date period. NOAA documents collection interruptions, so a small or empty day requires review and cannot automatically be labelled incomplete or low traffic. Observed timestamp bounds do not resolve any of those questions or AIS receiver completeness.

The local handling and manifest policy is in [../data/README.md](../data/README.md). The July evidence passed independent audit and satisfied ADR 0017's acceptance condition. Sequential author-submitted August--November calendar-month extracts are authorized under the same controls; August and September have been processed and October is the next request. One combined later-period request is not authorized, and later-month or full-period safety is not established in advance.
...
- **Later-month and full-period AccessAIS evidence.** ADR 0017 accepts AccessAIS
  as the preferred route after the exact July monthly gate, and August and
  September have since been processed under the same controls. Independent
  transfer completeness and October--November/full-period processing safety
