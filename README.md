...
| Processing workflow | **In progress** — locked Python package, DuckDB engine, contracts, validators, a bounded local multi-date AccessAIS intake and resumable daily-cleaning path exercised for backward compatibility with one real direct CSV, deterministic projected water-grid generation, and deterministic tested whale-grid transfer with byte-identical reruns; no real multi-date delivery has been exercised, and transfer completeness, analytical-period AIS retrieval, vessel aggregation, and exposure processing remain unfinished |
...
The accepted hybrid direction uses Python as the reproducible processing and analytical core, QGIS for local inspection and required visual verification, and a Next.js / TypeScript application using the [ArcGIS Maps SDK for JavaScript](https://developers.arcgis.com/javascript/latest/). Three project-layer publication candidates remain open: limited ArcGIS Location Platform feature/vector-tile/map-tile services within verified free-tier capacity, ArcGIS Online organization-hosted layers when account capabilities support them, and a non-Esri public fallback if neither fits. Local ArcGIS basemap access through a scoped browser API key is verified; deployed-origin access and project-layer hosting capabilities are not. No publication route has been implemented, and the project does not authorize paid usage.
