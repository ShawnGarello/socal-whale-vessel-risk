...
>
> **Nothing has been deployed or published.** The isolated Vercel Hobby project
> `socal-whale-vessel-overlap` and its reserved production hostname exist, but
> no deployment is assigned to that hostname and no project-derived layer is
> hosted anywhere. Route-specific plan, eligibility, basemap-capacity and
> browser-key checks passed on 2026-09-07. A checksum-bound candidate from
> merged `main` is staged and linked locally; explicit upload approval and
> deployed verification remain open.
