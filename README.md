...
| Processing workflow | **In progress** — locked Python package, DuckDB engine, contracts, validators, a bounded local multi-date AccessAIS intake and resumable daily-cleaning path exercised with overlapping real one-day and two-day deliveries, deterministic projected water-grid generation, deterministic tested whale-grid transfer with byte-identical reruns, and implemented/synthetically tested candidate vessel-grid aggregation exercised across the real two-day parameter matrix with deterministic repeats and corrected QGIS inspection; transfer completeness, safe monthly/full-period scaling, analytical-period AIS retrieval, accepted vessel thresholds, final period-wide vessel aggregation, the production vessel input, and exposure processing remain unfinished |
