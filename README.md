...
| Processing workflow | **In progress** — locked Python package, DuckDB engine, contracts, validators, and a bounded local multi-date AccessAIS intake with resumable resource-controlled daily cleaning exercised through one-day, two-day, seven-day, July monthly, August monthly, and September monthly deliveries; the July run reconciled 17,998,955 rows, the August run 18,284,354 rows, and the September run 15,638,516 rows, each across exactly its own requested dates, and each retry reused every date, leaving 92 of 153 compatible dates with no conflicts; ADR 0017 accepts sequential author-submitted August--November monthly AccessAIS extracts under the same controls, while publisher-side byte completeness, observational completeness, October--November/full-period safety, and analytical-period retrieval remain `unverified` or unfinished; deterministic projected water-grid generation, deterministic tested whale-grid transfer with byte-identical reruns, and implemented/synthetically tested candidate vessel-grid aggregation exercised across the real two-day parameter matrix with deterministic repeats and corrected QGIS inspection; accepted vessel thresholds, final period-wide vessel aggregation, the production vessel input, and exposure processing remain unfinished |
